import { LightStepGetCommitCoverageArgs } from './api'
...
/**
 * Returns the URL parameters used to access the LightStep API for the URI's repository.
 *
 * Currently only GitHub.com repositories are supported.
 */
export function lightstepParamsForRepositoryCommit(
  uri: Pick<ResolvedURI, 'repo' | 'rev'>
): Pick<LightStepGetCommitCoverageArgs, 'service' | 'owner' | 'repo' | 'sha'> {
